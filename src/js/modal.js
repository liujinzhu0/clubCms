define(["jquery", "vue","daterangepicker","moment"], function($, Vue) {
	...
	var vm = new Vue({
		...
		data: {
			...
			awardTmpl:0,//奖励模板
			mrjl:[
			{idx:1,type:0,num:null},
			{idx:2,type:0,num:null},
			{idx:3,type:0,num:null}
			]//初始默认奖励数组
		},
		methods: {
			back: function(tag) {
			...
			saveAndPublic: function() {
				console.log("保存并发布比赛")
			},
			close: function(tag) {
				$("#" + tag).modal("hide")
			},
			createTmpl: function(tag) {
			...
			basicInfo: function(tag) {
				this.create = tag;

			},
			selectTmpl: function(tag) {
				this.tmplType = tag;
			...
			mrjlMethod:function(key,type){
				console.log(key)
				console.log(type)
			},
			addRanking:function(type){
				//添加名次
		...
		watch:{
			awardType:function(newVal,oldVal){
